...
describe('GET /todos/:id', () => {
  ...
  it('should return todo with provided id', (done) => {
    request(app)
      ...
      .set('x-auth', users[0].tokens[0].token) // todos[0] is owned by users[0]
      .expect(200)
      .expect((res) => {
  ...
  it('should not return todo created by other user', (done) => {
    request(app)
      ...
      .set('x-auth', users[0].tokens[0].token) // todos[1] is not owned by users[0]
      .expect(404)
      .end(done);
  });
...
describe('PATCH /todos/:id', () => {
  ...
  it('should not update and complete todo for a different user', (done) => {
    var text = 'todo text updated via PATCH /todos/:id 1'
    request(app)
      .patch(`/todos/${todos[0]._id.toHexString()}`)
      .set('x-auth', users[1].tokens[0].token)
